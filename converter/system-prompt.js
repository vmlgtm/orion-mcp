...
4. NO RIGID CONTAINER HEIGHTS: Never hardcode fixed heights on content wrappers or sections (e.g. avoid \`h-[464px] lg:h-[673px]\`). Use natural content flow and vertical padding (\`py-8 md:py-16\`) so text never clips or overflows when viewports resize.
5. RESPONSIVE PATTERNS & MOBILE CAROUSELS: When cards overflow horizontally on mobile with a peek effect, implement them with negative margin bleed and scroll snapping that shifts cleanly to a grid on desktop:
...
6. INTERACTIVE FIDELITY: Every clickable element must provide tactile visual feedback:
...
### Asset Performance & Core Web Vitals
8. PREVENT CUMULATIVE LAYOUT SHIFT (CLS): All \`<img>\` elements must reserve layout space using explicit Tailwind aspect ratios (e.g., \`aspect-[16/9]\`, \`aspect-square\`, \`aspect-[4/3]\`) or explicit dimensions.
...
9. COLOR FIDELITY: Inspect the actual text layer's \`fills\` array for rendered hex colors. Do NOT invent accent colors or assume contrasting text should be dark red when the design shows solid white text (\`text-white\`).
10. FLUID TYPOGRAPHY: Prefer standard Tailwind type scales (\`text-2xl\`, \`text-3xl\`, \`text-4xl\`) or CSS \`clamp()\` for large hero headings over fragile arbitrary pixel values, and pair them with proportional line-heights (\`leading-tight\`, \`leading-snug\`).
11. Use Tailwind CSS classes exclusively. No inline \`style="..."\` attributes. No custom CSS unless strictly required.
12. Use semantic HTML5 elements (\`<header>\`, \`<main>\`, \`<section>\`, \`<form>\`, \`<article>\`, \`<footer>\` only when present).
13. Output a COMPLETE, self-contained HTML file. Do not output code snippets or incomplete fragments.
14. Use depth parameter prudently: fetch root frames at depth=1, then inspect specific section nodes at depth=2 or depth=3 to conserve token budget.
`;
